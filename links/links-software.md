# GPG
- [Operational PGP](https://gist.github.com/grugq/03167bed45e774551155)
- [GPGSync](https://github.com/firstlookmedia/gpgsync)
- [Keylist RFC: Distributing OpenPGP Keys with Signed Keylist Subscriptions](https://tech.firstlook.media/keylist-rfc-explainer) 

# GPG/SSH agents
- [Archlinux wiki: GNOME/Keyring](https://wiki.archlinux.org/index.php/GNOME/Keyring)
- [Using gpg-agent Effectively](https://eklitzke.org/using-gpg-agent-effectively)

# Hashcat
- [Example hashes](https://hashcat.net/wiki/doku.php?id=example_hashes)

# Nginx
- [Nginx Admin's Handbook](https://github.com/trimstray/nginx-admins-handbook)
- [Tuning NGINX for Better Performance](https://blog.codeship.com/tuning-nginx/)
- [NGINX Tuning For Best Performance](https://github.com/denji/nginx-tuning)
- [Apache vs. Nginx: Pros & Cons for WordPress](https://code.tutsplus.com/tutorials/apache-vs-nginx-pros-cons-for-wordpress--cms-28540)
- [Wordprocess.org: Nginx](https://wordpress.org/support/article/nginx/)
- [WordPress Nginx Configuration Templates](https://github.com/pothi/wordpress-nginx)
- [Getting Started with NGINX - Part 1: Installation and Basic Setup](https://linode.com/docs/web-servers/nginx/nginx-installation-and-basic-setup/)
- [Optimal value for Nginx worker_connections](https://serverfault.com/questions/787919/optimal-value-for-nginx-worker-connections)
- [How To Optimize Nginx Configuration](https://www.digitalocean.com/community/tutorials/how-to-optimize-nginx-configuration)
- [Nginx.com: 9 Tips for Improving WordPress Performance](https://www.nginx.com/blog/9-tips-for-improving-wordpress-performance-with-nginx/)
- [Nginx.com: Tuning NGINX for Performance](https://www.nginx.com/blog/tuning-nginx/)
- [Nginx.com: Pitfalls and Common Mistakes](https://www.nginx.com/resources/wiki/start/topics/tutorials/config_pitfalls/?highlight=pitfalls)
- [Strong SSL Security on nginx](https://raymii.org/s/tutorials/Strong_SSL_Security_On_nginx.html)

# OpenSSL
- [Strong Ciphers for Apache, nginx and Lighttpd](https://cipherli.st/)
- [Mozilla SSL Configuration Generator](https://mozilla.github.io/server-side-tls/ssl-config-generator/)
- [Security/Server Side TLS](https://wiki.mozilla.org/Security/Server_Side_TLS)

# OpenVPN
- [16 tips on OpenVPN security ](https://blog.g3rt.nl/openvpn-security-tips.html)

# SSH
- [Archlinux wiki: SSH Keys](https://wiki.archlinux.org/index.php/SSH_keys)
- [Hardening SSH with 2fa](https://gist.github.com/lizthegrey/9c21673f33186a9cc775464afbdce820)
- [SSH Agent Forwarding considered harmful](https://heipei.io/2015/02/26/SSH-Agent-Forwarding-considered-harmful/)

# systemd
- [Rethinking PID 1](http://0pointer.net/blog/projects/systemd.html)
- [The Tragedy of systemd](https://www.youtube.com/watch?v=6AeWu1fZ7bY)

# tmux
- [The Tao of tmux](https://leanpub.com/the-tao-of-tmux/read)
- [Tmux and Vim - even better together](https://www.bugsnag.com/blog/tmux-and-vim)

# UEFI
- [Understanding modern UEFI-based platform boot](https://depletionmode.com/uefi-boot.html)

# Vim
- [You Should Be Using Tags In Vim](https://vimways.org/2018/you-should-be-using-tags-in-vim/)
- [History and effective use of Vim](https://begriffs.com/posts/2019-07-19-history-use-vim.html?hn=3)

# Wireguard
- [Some Unofficial WireGuard Documentation](https://docs.sweeting.me/s/wireguard#)
# Wordpress
- [Wordpress permalinks](http://nginxlibrary.com/wordpress-permalinks/)
- [Using Permalinks](https://wordpress.org/support/article/using-permalinks/)
- [A Look At The Modern WordPress Server Stack](https://www.smashingmagazine.com/2016/05/modern-wordpress-server-stack/)

## Permissions
- [Strict file ownership for your WordPress installation](https://bjornjohansen.no/strict-file-ownership-for-wordpress)
- [Changing File Permissions](https://wordpress.org/support/article/changing-file-permissions/)
- [Configuring Automatic Background Updates](https://wordpress.org/support/article/configuring-automatic-background-updates/)
- [Hardening WordPress](https://wordpress.org/support/article/hardening-wordpress/)
- [Updating WordPress](https://wordpress.org/support/article/updating-wordpress/)
- [Fix wordpress file permissions ](https://gist.github.com/Adirael/3383404)
- [Correct file permissions for WordPress](https://stackoverflow.com/questions/18352682/correct-file-permissions-for-wordpress)
- [1.5: How to Harden Your WordPress Site From Attacks](https://www.wordfence.com/learn/how-to-harden-wordpress-sites/)
